...
**Symlinked to standard locations:**

- `config` → `~/.config/i3/config` - Main i3 configuration
- `.i3blocks.conf` → `~/.config/i3blocks/config` - Status bar blocks
- `.xinitrc` → `~/.xinitrc` - X11 session initialization
- `.Xresources` → `~/.Xresources` - Terminal colors and settings
...
Your X11 session will start with i3 as the window manager.
